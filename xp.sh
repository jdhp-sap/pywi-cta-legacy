...
#
#WT_MR_FILTER_PARAMS="-K -k -C1 -m3 -s1,2,3,4 -n4"  # 2016-10-19 REFERENCE
...
WT_MR_FILTER_PARAMS="-K -k -C1 -m3 -s3 -n4 --kill-isolated-pixels"  # 2016-10-19 REFERENCE
echo "* GAMMA WAVELETS" & ./datapipe/denoising/wavelets_mrfilter.py -b all --label="WT-K-k-C1-m3-s3-n4" ${WT_MR_FILTER_PARAMS}      -o score_gamma_all_wavelets_mrfilter_ref.json    ~/astri_data/fits/gamma 2>&1 | tee score_gamma_all_wavelets_mrfilter_ref.json.log
for FILE in .tmp*.fits ; do rm $FILE ; done
...
WT_MR_FILTER_PARAMS="-K -k -C1 -m3 -s2,2,3,3 -n4 --kill-isolated-pixels"  # 2016-10-19 REFERENCE
echo "* GAMMA WAVELETS" & ./datapipe/denoising/wavelets_mrfilter.py -b all --label="WT-K-k-C1-m3-s3-n4" ${WT_MR_FILTER_PARAMS}      -o score_gamma_all_wavelets_mrfilter_ref.json    ~/astri_data/fits/gamma 2>&1 | tee score_gamma_all_wavelets_mrfilter_ref.json.log
for FILE in .tmp*.fits ; do rm $FILE ; done
...
WT_MR_FILTER_PARAMS="-K -k -C1 -m3 -s3 -n4 --kill-isolated-pixels"  # 2016-10-19 REFERENCE
echo "* PROTON WAVELETS" & ./datapipe/denoising/wavelets_mrfilter.py -b all --label="WT-K-k-C1-m3-s3-n4" ${WT_MR_FILTER_PARAMS}     -o score_proton_all_wavelets_mrfilter_ref.json    ~/astri_data/fits/proton 2>&1 | tee score_proton_all_wavelets_mrfilter_ref.json.log
for FILE in .tmp*.fits ; do rm $FILE ; done
